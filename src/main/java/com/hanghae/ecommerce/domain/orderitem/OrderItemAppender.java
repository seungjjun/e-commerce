...
public class OrderItemAppender {
	...
	public List<OrderItem> create(
		...
		for (OrderRequest.ProductOrderRequest orderRequest : productOrderRequests) {
			Product product = products.stream().filter(p -> p.id().equals(orderRequest.id())).findFirst()
				.orElseThrow(() -> new EntityNotFoundException(orderRequest.id() + " 상품의 정보를 찾지 못했습니다."));

			orderItemEntities.add(new OrderItemEntity(order.id(), product.id(), product.name(), product.price(),
				product.orderTotalPrice(orderRequest.quantity()), orderRequest.quantity()));
		}
