...
class StockServiceTest {
	...
	@Test
	@DisplayName("상품 재고 보상 함수를 호출한다.")
	void compensate_product_stock() {
		// Given
		...
		OrderRequest request = new OrderRequest(
				...
				List.of(new OrderRequest.ProductOrderRequest(1L, 5L)),
				50_000L,
				"CARD"
		);

		// When
		stockService.compensateStock(decreasedStocks, request);

		// Then
		verify(stockUpdator, atLeastOnce()).compensateStock(anyList(), anyList());
	}
